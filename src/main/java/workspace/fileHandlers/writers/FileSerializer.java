...
public abstract class FileSerializer {
    ...
    public void serializeEntities(int maxRecords, String fileName, List<Entity> entitiesList) throws IOException {
        int fileNo = 1;
        String generatedFileName = FileNameGenerator.generateFileName(fileName);
        for (int i = 0; i < entitiesList.size(); i += maxRecords) {
            this.serializeSingleFile(String.format(generatedFileName, fileNo), entitiesList.subList(i,
                    Math.min(entitiesList.size(), i + maxRecords)));
            fileNo++;
        }
    }
}
